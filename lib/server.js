require('./errors');
var Lenr = require('./lenr');
var LenrREST = require('./lenr/rest');
var LenrWebsocket = require('./lenr/websocket');
var LenrDB = require('./lenr/db');
...
module.exports = (function() {
  ...
  function Server(config) {
    ...
    function init(err, db) {
      ...
      var lenr = new Lenr(db, configHash.lenr);
      var authentication = configHash.authentication ? new Authentication(configHash.authentication) : null;
      var rest = new LenrREST(lenr);
      var websocket = new LenrWebsocket(lenr, authentication);
      var app = express();
      ...
      getLog().info('LENR API is ready and listening on port ' + configHash.port);
      getLog().debug('With config', configHash);
    }

    new LenrDB(configHash.mongodb, init.bind(this));
  }
  ...
  Server.prototype.installHandlers = function(app) {
    ...
    app.use('/web(?:/job/:id)?', express.static(publicDir, {
      etag: false
    }));
